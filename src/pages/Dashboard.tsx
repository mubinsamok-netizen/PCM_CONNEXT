...
export const DashboardScreen = ({ onSelectProject }: { onSelectProject: (p: any) => void }) => {
  ...
  return (
    <main className="flex-grow w-full px-4 sm:px-6 lg:px-8 xl:px-12 pt-6 pb-10 md:pt-8 md:pb-14 block">
       ...
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
          <div>
             <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">เลือกโครงการ (Select Project)</h1>
             <p className="text-gray-500 text-sm md:text-base">ยินดีต้อนรับกลับมา กรุณาเลือกโครงการที่คุณต้องการจัดการ</p>
          </div>
          
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
             {/* Filters */}
             <button onClick={() => setActiveView('vo_summary')} className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold px-4 py-2.5 rounded-xl shadow-sm flex items-center justify-center gap-2 whitespace-nowrap h-full">
                <FileText size={18} />
                ดูรายงาน VO ภาพรวม
             </button>
             <div className="flex flex-1 sm:flex-none flex-wrap items-center bg-white border border-gray-200 rounded-xl px-3 py-2 shadow-sm gap-2 sm:gap-3">
                ...
                {/* Status Filter */}
                <select className="text-sm outline-none bg-transparent text-gray-600 border-r border-gray-200 pr-3 cursor-pointer">
                ...
                {/* SE Name Filter */}
                <input type="text" placeholder="ชื่อวิศวกร (SE)..." className="text-sm outline-none bg-transparent text-gray-600 w-full sm:w-32 placeholder-gray-400 mt-2 sm:mt-0 border-t sm:border-t-0 border-gray-100 pt-2 sm:pt-0" />
             </div>
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 md:gap-8">
          {projects.map((p) => {
             ...
             return (
               <a key={p.id} href="#" onClick={(e) => { e.preventDefault(); onSelectProject(p); }} className="relative rounded-xl p-[2px] bg-white overflow-hidden shadow-lg border border-gray-100 cursor-pointer group flex flex-col h-full hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                  <div className="relative z-10 bg-white h-full w-full rounded-[10px] overflow-hidden flex flex-col">
                      ...
                      <div className="p-6 flex-grow flex flex-col">
                          <h3 className="text-[22px] font-bold text-gray-800 mb-1.5 leading-tight group-hover:text-brand-600 transition-colors">{p.name}</h3>
                          <div className="flex items-center text-sm text-gray-500 mb-3 gap-1.5 font-light">
                              <MapPin className="w-4 h-4 text-brand-500 shrink-0 mt-0.5" />
                              <span className="truncate leading-tight">{p.location}</span>
                          </div>
                          <div className="flex flex-col gap-2.5 mb-4 px-1.5 border-l-2 border-gray-100 py-1">
                              <div className="flex items-center justify-between text-[13.5px] text-gray-600 w-full gap-2">
                                  <div className="flex items-center gap-1.5 font-medium shrink-0">
                                      <span>วิศวกร</span>
                                  </div>
                                  <span className="text-brand-700 truncate font-bold text-[14px]">{p.seName || p.se || '-'}</span>
                              </div>
                              <div className="flex items-center justify-between text-[13.5px] text-gray-600 w-full gap-2">
                                  <span className="text-gray-700 truncate text-[12.5px] mt-0.5 tracking-tight w-full flex justify-between pr-0.5">
                                      <span>เริ่มต้น <strong className="text-gray-900 font-semibold">{p.start}</strong></span>
                                      <span>สิ้นสุด <strong className="text-gray-900 font-semibold">{p.end}</strong></span>
                                  </span>
                              </div>
                          </div>
                          <div className="w-full mb-5 z-10">
                              <div className="flex justify-between text-xs font-semibold text-gray-600 mb-1.5 px-0.5"><span>ความคืบหน้าโครงการ</span></div>
                              <div className="relative flex items-center w-full bg-gray-500/20 h-5 rounded-full overflow-hidden shadow-inner">
                                  <div className={`${colorTheme.bg} h-5 rounded-full transition-all duration-1000`} style={{ width: `${p.progress}%` }}></div>
                                  <span className={`absolute inset-0 flex items-center justify-center text-[11px] font-bold ${p.progress > 50 ? 'text-white drop-shadow-md' : 'text-gray-700'}`}>{p.progress}%</span>
                              </div>
                          </div>
                          <div className="flex justify-between items-center border-t border-gray-100 pt-4 mt-auto">
                              <div className="flex -space-x-2.5">
                                  <img className="w-8 h-8 rounded-full border-2 border-white object-cover bg-gray-200" src={p.pm || `https://ui-avatars.com/api/?name=${encodeURIComponent(p.pmName || 'PM')}&background=3b82f6&color=fff`} alt={p.pmName || 'PM'} title={p.pmName || 'PM'} referrerPolicy="no-referrer" />
                                  <img className="w-8 h-8 rounded-full border-2 border-white object-cover bg-gray-200" src={p.se || `https://ui-avatars.com/api/?name=${encodeURIComponent(p.seName || 'SE')}&background=10b981&color=fff`} alt={p.seName || 'SE'} title={p.seName || 'SE'} referrerPolicy="no-referrer" />
                              </div>
