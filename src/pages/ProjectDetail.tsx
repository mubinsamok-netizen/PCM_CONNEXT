...
export const ProjectDetailScreen = ({ project, onBack, user }: { project: Project, onBack: () => void, user: any }) => {
  ...
  return (
    <main className="flex-grow flex flex-col md:flex-row h-[calc(100vh-80px)] overflow-hidden bg-[#FAFAFA]">
      <aside className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-gray-100 flex-shrink-0 overflow-y-auto custom-scrollbar">
         <nav className="flex md:flex-col p-2 md:p-3 space-x-2 md:space-x-0 md:space-y-1">
            <div className="hidden md:flex p-4 mx-2 mb-2 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 flex-col" onClick={onBack}>
               <div className="text-xs font-bold text-gray-500 mb-1 flex items-center gap-1"><ArrowLeft size={14}/> กลับหน้าแรก</div>
               <div className="font-bold text-gray-900 truncate" title={project.name}>{project.name}</div>
            </div>
            {TABS.map(tab => {
               ...
               return (
                 <div key={tab.id} className="flex flex-col">
                   <button 
                      ...
                      className={`flex items-center justify-between px-4 py-3 rounded-lg text-[15px] font-medium transition-all whitespace-nowrap md:whitespace-normal border-l-4 ${isActive && !tab.subItems || activeTab===tab.id ? 'bg-red-50/50 text-gray-900 font-semibold border-brand-500' : 'text-gray-500 hover:bg-gray-50 border-transparent hover:text-gray-800'}`}
                   >
                      <div className="flex items-center gap-3">
                        <tab.icon size={20} className={`shrink-0 ${isActive ? 'text-brand-600 opacity-100' : 'opacity-70'}`} />
                        <span className="hidden md:inline">{tab.name}</span>
                      </div>
                      {tab.subItems && (
                          <div className="hidden md:flex items-center justify-center w-6 h-6 rounded-full hover:bg-gray-200">
                             {isExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                          </div>
                      )}
                   </button>
                   {tab.subItems && isExpanded && (
                       <div className="hidden md:flex flex-col ml-8 mt-1 space-y-1 border-l-2 border-gray-100 pl-3">
                          {tab.subItems.map(subItem => (
                              <button
                                 key={subItem.id}
                                 onClick={() => setActiveTab(subItem.id)}
                                 className={`text-left px-3 py-2 rounded-lg text-[14px] font-medium transition-colors ${activeTab === subItem.id ? 'bg-red-50/50 text-gray-900 font-semibold' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
                              >
                                 {subItem.name}
                              </button>
                          ))}
      ...
      <section className="flex-grow overflow-y-auto custom-scrollbar p-4 md:p-8">
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 md:mb-8">
             <div className="flex flex-col border-b border-gray-200 pb-4 w-full md:border-none md:pb-0">
                 <div className="flex items-center gap-3 mb-1">
                     <button onClick={onBack} className="md:hidden text-gray-500 p-2"><ArrowLeft size={20}/></button>
                 ...
                 <div className="flex items-center text-sm text-gray-500 font-light gap-1.5 md:ml-0 ml-10 mt-1">
