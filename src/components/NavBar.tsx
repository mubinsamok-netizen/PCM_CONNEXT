...
import { Search, Bell, Settings, Maximize, Minimize, LogOut, ChevronDown, Wallet, CheckCircle, AlertTriangle, Info, X } from 'lucide-react';
...
export const NavBar = ({ user, onLogout, onGoHome, onGoAdmin, onGoProfile, onGoPayment }: { user: User, onLogout: () => void, onGoHome: () => void, onGoAdmin?: () => void, onGoProfile?: () => void, onGoPayment?: () => void }) => {
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-gray-100 w-full sticky top-0 z-50 h-20 flex items-center transition-opacity duration-300">
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 flex justify-between items-center">
        ...
        <div className="hidden sm:flex flex-1 items-center justify-center px-4 lg:px-8">
          <form onSubmit={handleSearch} className="flex items-center border pl-4 gap-2 bg-white border-gray-200 hover:border-gray-300 focus-within:border-brand-500 focus-within:ring-4 focus-within:ring-brand-500/10 transition-all h-[46px] rounded-full overflow-hidden max-w-md w-full shadow-sm">
            ...
            <input 
              ...
              className="w-full h-full outline-none text-[14px] text-gray-700 bg-transparent placeholder-gray-400" 
            />
        ...
        <div className="flex items-center gap-3 sm:gap-4 shrink-0">
           ...
           {user.role === 'admin' && (
             <button onClick={() => { if(onGoAdmin) onGoAdmin(); }} className="hidden sm:block p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 rounded-xl transition-colors cursor-pointer shrink-0" title="จัดการผู้ใช้งาน">
                <Settings size={20} className="stroke-[1.8]" />
             </button>
           )}
           
           <button onClick={toggleFullscreen} className="hidden sm:block p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600 rounded-xl transition-colors cursor-pointer shrink-0">
           ...
           <div className="hidden sm:flex flex-col justify-center items-end mr-1">
              <span className="text-[14px] font-bold text-gray-800 leading-none tracking-tight font-mono">
           ...
           <div onClick={() => { if(onGoProfile) onGoProfile(); }} className="hidden sm:flex items-center gap-3 cursor-pointer hover:bg-gray-50 p-1.5 pr-2.5 rounded-[1.25rem] transition-colors border border-transparent hover:border-gray-100">
              <img src={user.avatar} alt="Avatar" className="w-10 h-10 rounded-[0.85rem] object-cover bg-gray-100 border border-gray-100/50" referrerPolicy="no-referrer" />
              <div className="flex flex-col justify-center">
                 <div className="flex items-center gap-1.5">
                    <span className="text-[14px] font-semibold text-gray-800 tracking-tight leading-none">{user.name}</span>
           ...
           <div className="h-6 w-px bg-gray-200 hidden sm:block ml-1"></div>
           <button onClick={() => {
             toast.success('ออกจากระบบสำเร็จ');
             onLogout();
           }} className="p-2 text-gray-400 hover:bg-red-50 hover:text-red-500 rounded-xl transition-colors cursor-pointer shrink-0">
