...
import { Download, AlertTriangle, ShieldAlert, CheckCircle, FileText, Server, Clock, CalendarDays, Eye } from 'lucide-react';

export const PlanTab = ({ project, user }: { project: Project, user: any }) => {
  const [activePlanType, setActivePlanType] = useState<'master' | 'actual' | '3week'>('master');
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex space-x-1 border-b border-gray-200">
         {!isContractor && (
           <>
             <button 
                ...
                className={`py-3 px-5 border-b-2 font-bold text-sm transition-colors flex items-center gap-2 ${activePlanType === 'master' ? 'border-brand-600 text-brand-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
             >
                <Server size={16}/> Master Plan
             </button>
             <button 
                onClick={() => setActivePlanType('actual')}
                className={`py-3 px-5 border-b-2 font-bold text-sm transition-colors flex items-center gap-2 ${activePlanType === 'actual' ? 'border-brand-600 text-brand-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
             >
         ...
         <button 
            ...
            className={`py-3 px-5 border-b-2 font-bold text-sm transition-colors flex items-center gap-2 ${activePlanType === '3week' ? 'border-brand-600 text-brand-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
         >
         ...
         <p className="text-sm text-gray-600 font-medium tracking-tight">
             แสดงข้อมูล: <span className="font-bold text-brand-700">
                 {activePlanType === 'master' ? 'แผนงานหลัก (Baseline v1.0)' : 
                  activePlanType === 'actual' ? 'ความคืบหน้าจริง (Real-time)' : 'แผนงาน 3 สัปดาห์ล่วงหน้า'}
             </span>
         </p>
